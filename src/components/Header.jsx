...
export default function Header({
  ...
  return (
    <header className="sticky top-0 z-50 glass-panel border-b border-slate-700/50 px-4 py-3">
            ...
            <div className="absolute top-full left-0 right-0 mt-1 glass-panel py-1 shadow-xl max-h-64 overflow-y-auto z-50">
              {results.map((r) => (
                <button key={r.id} onClick={() => selectLocation(r)}
                  className="w-full text-left px-3 py-2.5 hover:bg-slate-700/50 flex items-center gap-2 transition-colors">
                  <MapPin size={14} className="text-slate-400 shrink-0" />
                  <div>
                    <div className="text-sm text-slate-200">{r.name}</div>
                    <div className="text-xs text-slate-500">{r.admin1}{r.country ? `, ${r.country}` : ''}</div>
                  </div>
                </button>
              ))}
            </div>
          ...
          <div className="relative">
            ...
            {showSaved && savedLocations.length > 0 && (
              <div className="absolute top-full right-0 mt-1 glass-panel py-1 shadow-xl w-56 z-50">
                <div className="px-3 py-1.5 text-xs font-medium text-slate-500 uppercase tracking-wider">Saved Locations</div>
                {savedLocations.map(s => (
                  <button key={s.id} onClick={() => { selectLocation(s); setShowSaved(false); }}
                    ...
                    <span className="text-sm text-slate-300">{s.label || s.name}</span>
      ...
      {location && (
        <div className="max-w-7xl mx-auto mt-1 flex items-center gap-2 text-xs text-slate-500">
          <MapPin size={10} />
